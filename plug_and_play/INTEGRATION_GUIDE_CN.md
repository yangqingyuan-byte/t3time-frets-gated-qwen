...
本指南详细说明如何将 FreDF 的频率域损失函数集成到您自己的时间序列预测模型中。
...
for epoch in range(num_epochs):
    for batch_x, batch_y in train_loader:
        ...
        if step % 100 == 0:
            ...
            print(f'  时域损失: {loss_temporal.item():.4f}')
            print(f'  频域损失: {loss_frequency.item():.4f}')
...
- **起始值**：0.5
- **调整建议**：
  - 如果频率损失过大，可以降低（如 0.1-0.3）
  - 如果效果不明显，可以增加（如 0.7-1.0）
...
# 检查梯度是否存在
print(f"模型参数梯度存在: {model.parameters().__next__().grad is not None}")
```
