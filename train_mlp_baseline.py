"""
训练简单的三层 MLP baseline，用于对比 VMD/FFT 等复杂模型的效果
"""
import os
import time
import random
...
def parse_args():
    ...
    parser.add_argument("--save", type=str, default="./logs_mlp/", help="save path")
...
def main():
    ...
    save_dir = os.path.join(
        args.save,
        args.data_path,
        f"mlp_i{args.seq_len}_o{args.pred_len}_h{args.hidden_dim}_"
        f"lr{args.learning_rate}_dn{args.dropout}_bs{args.batch_size}_seed{args.seed}/",
    )
    ...
    try:
        ...
        log("Start training MLP baseline...", flush=True)
